...
/*changes done 14-06-2021 
public function sendEmail($data,$body)
	{
		
		$to = "testing0415048@gmail.com";
		$subject = "Ishtika Agastya ".$data['source']." ".$data['campaign']." Enquiry";
		$headers = "MIME-Version: 1.0" . "\r\n";
		$headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
		$headers .= 'From: noreply@ishtika.in' . "\r\n";
		$headers .= 'Bcc: satish.d@akriveiatech.com,sajid@akriveiatech.com' . "\r\n";
...
if($("#contact-form").length){
    $("#contact-form").validate({
        submitHandler: function(form) {
          ...
          $(form).ajaxSubmit({
            dataType:  'json',
			url: 'contactus-formvalidation.php',
            success: function(data) {
			  ...
			  if( data.success ) {	  
				...
				$(form).find('.txtFromelement').each(function(){
					$(this).val('');
				});
				...
				setTimeout(function(){ $(form_result_div).fadeOut('slow') }, 6000);
				 swal("Success!",data.message, "success");
				  ...
				  form_btn.prop('disabled', false).html(form_btn_old_msg);
				  $(form_error_div).html(data.error).fadeIn('slow');
				  setTimeout(function(){ $(form_error_div).fadeOut('slow') }, 6000);
				  swal("Message!",data.error, "warning");
